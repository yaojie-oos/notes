...
public class UserController {
	...
	public JSONPObject findUserByTicket(@PathVariable String ticket,
		...
		String originalIP = jedisCluster.hget(username, "IP");
		if(ip.equals(originalIP)) {
			if(jedisCluster.hget(username,"TICKET").equals(ticket)) {
				String userJSON = jedisCluster.hget(username, "USER");
				if(!StringUtils.isEmpty(userJSON)) {
					return new JSONPObject(callback, SysResult.success(userJSON));
				}
			}
		}
		return new JSONPObject(callback, SysResult.fail("用户信息不正确!!"));
	}
